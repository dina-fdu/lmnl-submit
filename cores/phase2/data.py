...
def get_dataloaders(dataset, batch, batch_unsup, dataroot, with_noise=True, random_state=0, unsup_idx=set()):
    ...
    trainloader = torch.utils.data.DataLoader(
        trainset, batch_size=batch, shuffle=False, num_workers=1, pin_memory=True,
        sampler=StratifiedSampler(trainset.targets), drop_last=True)

    unsuploader = torch.utils.data.DataLoader(
        otherset, batch_size=batch_unsup, shuffle=True, num_workers=2, pin_memory=True,
        sampler=None, drop_last=True)

    testloader = torch.utils.data.DataLoader(
        testset, batch_size=256, shuffle=False, num_workers=1, pin_memory=False,
        drop_last=False
    )
