...
def train_and_eval(tag, dataroot, metric='last', resume=False, save_path=None, only_eval=False, unsupervised=False, devices=None):
    ...
    save_path = '../../' + C.get()['model']['noise_type'] + '_learning.pt'
    ...
    if lr_scheduler_type == 'cosine':
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=t_max, eta_min=C.get()['lr'])
    else:
    ...
    for epoch in range(epoch_start, max_epoch + 1 + 10):
        if epoch == max_epoch:
            scheduler = None
        model.train()
        rs = dict()
        ...
        model.eval()
        if epoch % (1 if 'cifar' in C.get()['dataset'] else 30) == 0 or epoch == max_epoch:
