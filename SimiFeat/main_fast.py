...
def set_model_min(config):
    ...
    state_dict = torch.load(f"../dividemix/dividemix/{config.dataset}_{config.noise_type}{config.lambda_u}best.pth.tar", map_location = "cpu")
...
if __name__ == "__main__":
    ...
    np.save(f'results/{config.method}_{config.dataset}_{config.noise_type}{config.lambda_u}_pred.npy', sel_noisy_summary)
    np.save(f'results/{config.method}_{config.dataset}_{config.noise_type}{config.lambda_u}_label.npy', train_dataset.train_noisy_labels)
