...
parser.add_argument("--pre_type", type=str, default='dividemix')  # image, cifar
...
def set_model_min(config):
    ...
    model = ResNet34_detect(num_classes=config.num_classes)
    state_dict = torch.load(f"../{config.noise_type}_learning.pt", map_location = "cpu")['state_dict']

    from collections import OrderedDict # muti GPU --> single GPU
...
if __name__ == "__main__":
    ...
    for loop_i in range(1):
        ...
        for epoch in range(config.num_epoch):
            ...
            if epoch % 1 == 0:
                ...
                aa[nan_flag] = 0
                # aa += 0.2  # if we have some offset  +0.1 improve recall?

                # sel_clean_summary = np.round(aa).astype(bool)
                # sel_noisy_summary = np.round(1.0 - aa).astype(bool)

                sel_clean_summary = aa > 0.5
                sel_noisy_summary = aa <= 0.5
    ...
    sel_noisy_summary = np.arange(50000)[sel_noisy_summary]
    np.save(f'../{config.dataset}_{config.noise_type}_detection.npy', sel_noisy_summary)
    # np.save(f'results/{config.dataset}_{config.noise_type}{config.lambda_u}_label.npy', train_dataset.train_noisy_labels)
